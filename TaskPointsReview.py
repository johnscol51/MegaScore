...
def run(DataDir):
    ...
    show_route_var = tk.BooleanVar(value=False)
    max_segment_var = tk.StringVar(value="15")

    tk.Label(options_frame, text="Output detail:").pack(side="left", padx=(0, 10))
    tk.Radiobutton(options_frame, text="Summary only", variable=show_route_var, value=False).pack(side="left")
    tk.Radiobutton(options_frame, text="Summary + route listing", variable=show_route_var, value=True).pack(side="left", padx=(10, 20))

    tk.Label(options_frame, text="Max segment distance (km):").pack(side="left")
    ...
    def process():
        ...
        for task_no in sorted(tasks.keys()):
            ...
            # -----------------------------------------------------
            if len(sp_list) == 1 and len(tp_list) == 1 and len(fp_list) == 0:
            ...
            emit_route_summary(
                ...
                max_segment_km, show_route_var.get(), append_line
            )
